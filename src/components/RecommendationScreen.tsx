...
const RecommendationScreen: React.FC<RecommendationScreenProps> = ({ userName, onPlanGenerated }) => {
  ...
  const handleGenerateRecommendation = async () => {
    ...
    try {
      ...
      // 3. Notificar éxito para cambiar de pestaña
  ...
  if (!formData) return <div className="p-10 text-center text-bocado-gray animate-pulse">Cargando perfil...</div>;
        ...
        <div className="flex-1 flex flex-col justify-center">
            <div className="space-y-6 w-full max-w-md mx-auto">
                <div className="text-center">
                    <h1 className="text-2xl font-bold text-bocado-dark-green tracking-tight">¡Hola, {userName}! 👋</h1>
                    <p className="text-bocado-gray mt-1">¿Dónde y qué quieres comer hoy?</p>
                </div>

                <div className="grid grid-cols-2 gap-4">
                    {EATING_HABITS.map(habit => (
                        <button key={habit} onClick={() => handleTypeChange(habit as any)} className={`flex flex-col items-center justify-center p-6 rounded-2xl border-2 transition-all duration-300 transform active:scale-95 ${recommendationType === habit ? 'bg-bocado-green text-white border-bocado-green shadow-lg' : 'bg-white text-gray-700 border-gray-100 hover:border-bocado-green/50'}`}>
            ...
            <div className={`transition-all duration-500 ease-in-out overflow-hidden ${recommendationType ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'}`}>
                <div className="overflow-y-auto no-scrollbar pt-4">
                    {recommendationType === 'En casa' ? (
                        <div className="space-y-4 animate-fade-in">
                             ...
                             <div className="grid grid-cols-2 gap-3">
                                {MEALS.map(meal => <button key={meal} type="button" onClick={() => setSelectedMeal(meal)} className={`py-3 px-2 rounded-xl border-2 text-sm font-bold transition-all ${selectedMeal === meal ? 'bg-bocado-green text-white border-bocado-green shadow-md' : 'bg-white text-gray-600 border-gray-50'}`}>{meal}</button>)}
                            </div>
                            {selectedMeal && (
                               <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100 mt-2 animate-slide-up">
                                    <div className="flex justify-between items-end mb-2"><label className="text-xs font-bold text-gray-400 uppercase tracking-wide">Tiempo de Cocina</label><span className="text-lg font-bold text-bocado-green">{renderCookingTime()}</span></div>
                        ...
                        <div className="space-y-4 animate-fade-in">
                            ...
                            <div className="grid grid-cols-2 gap-2.5">
                                {CRAVINGS.map(craving => <button key={craving} type="button" onClick={() => toggleCraving(craving)} className={`py-2.5 px-2 rounded-xl border text-[11px] font-bold transition-all ${selectedCravings.includes(craving) ? 'bg-bocado-green text-white border-bocado-green shadow-md' : 'bg-white text-gray-600 border-gray-50'}`}>{craving}</button>)}
                            </div>
                        </div>
                    )}
                    ...
                    <div className={`mt-8 pt-6 border-t border-gray-50 transition-opacity duration-500 ${isSelectionMade ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                        <button onClick={handleGenerateRecommendation} disabled={isGenerating} className="w-full bg-bocado-green text-white font-bold py-4 rounded-2xl text-lg shadow-xl shadow-green-100 hover:bg-bocado-dark-green transition-all active:scale-95 disabled:bg-gray-200 flex items-center justify-center gap-2">
...
export default RecommendationScreen; // ESTO ARREGLA EL ERROR DE VERCEL