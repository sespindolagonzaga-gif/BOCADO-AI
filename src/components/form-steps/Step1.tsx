...
interface ExtendedStep1Props extends FormStepProps {
  ...
  // Agregamos la prop para manejar el cambio de país con ambos valores
...
const Step1: React.FC<ExtendedStep1Props> = ({ 
  ...
  const handleCountrySelect = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    // Limpiamos ciudad local al cambiar país
    setLocalCityQuery('');
  ...
  const handleCitySearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    updateData('city', value); // Guardamos lo que escribe por si no elige de la lista
    if (onSearchCity) onSearchCity(value);
  };

  const handleSelectCity = (city: any) => {
    const cityName = city.name; // Solo el nombre de la ciudad para que se vea limpio
    ...
    <div className="space-y-5 animate-fade-in">
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input type="text" name="firstName" value={data.firstName} onChange={handleNameChange} placeholder="Ej. Juan" className={`w-full px-4 py-3 rounded-xl border-2 transition-all ${errors.firstName ? 'border-red-300 bg-red-50' : 'border-gray-100 focus:border-bocado-green focus:outline-none'}`} />
        </div>
        <div>
          <label className="block text-xs font-bold text-bocado-dark-green mb-1 ml-1 uppercase">Apellido(s)</label>
          <input type="text" name="lastName" value={data.lastName} onChange={handleNameChange} placeholder="Ej. Pérez" className={`w-full px-4 py-3 rounded-xl border-2 transition-all ${errors.lastName ? 'border-red-300 bg-red-50' : 'border-gray-100 focus:border-bocado-green focus:outline-none'}`} />
        </div>
      </div>
      ...
      <div className="relative">
        ...
        <input 
          ...
          className={`w-full px-4 py-3 rounded-xl border-2 transition-all ${errors.email ? 'border-red-300 bg-red-50' : 'border-gray-100 focus:border-bocado-green focus:outline-none'}`} 
        />
        {showEmailSuggestions && emailSuggestions.length > 0 && (
          <div className="absolute z-10 w-full mt-1 bg-white border border-gray-100 rounded-xl shadow-lg">
            {emailSuggestions.map((s) => (
              <div key={s} onClick={() => { updateData('email', s); setShowEmailSuggestions(false); }} className="px-4 py-2 text-sm hover:bg-gray-50 cursor-pointer border-b border-gray-50 last:border-0">{s}</div>
            ))}
          </div>
        )}
      </div>
      ...
      {!hidePasswordFields && (
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="password" name="password" value={data.password || ''} onChange={(e) => updateData('password', e.target.value)} className={`w-full px-4 py-3 rounded-xl border-2 transition-all ${errors.password ? 'border-red-300 bg-red-50' : 'border-gray-100 focus:border-bocado-green focus:outline-none'}`} />
          </div>
          <div>
            <label className="block text-xs font-bold text-bocado-dark-green mb-1 ml-1 uppercase">Confirmar</label>
            <input type="password" name="confirmPassword" value={data.confirmPassword || ''} onChange={(e) => updateData('confirmPassword', e.target.value)} className={`w-full px-4 py-3 rounded-xl border-2 transition-all ${errors.confirmPassword ? 'border-red-300 bg-red-50' : 'border-gray-100 focus:border-bocado-green focus:outline-none'}`} />
          </div>
        </div>
      )}
    </div>
